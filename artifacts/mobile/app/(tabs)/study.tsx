...
export default function StudyScreen() {
  ...
  const dateStrip = Array.from({ length: 7 }, (_, i) => {
    ...
    return { dateStr, label: dayLabel, dayNum, isSelected, hasStudy };
  });
  ...
  const handleStartSubject = (id: string, name: string) => {
    ...
    if (pomodoro) {
      Alert.alert("Session Active", `Stop "${pomodoro.subjectName}" first, or stop from the timer above.`, [{ text: "OK" }]);
      return;
    }
  ...
  const handleStop = () => {
    Alert.alert("Stop Session?", "Elapsed time (if any) will be saved.", [
      { text: "Cancel", style: "cancel" },
      { text: "Stop", style: "destructive", onPress: stopPomodoro },
    ]);
  };

  const showUndoBanner =
    lastSavedSession !== null &&
    lastSavedSession.id !== undoDismissed;
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerActions}>
            <TouchableOpacity
              ...
              style={[styles.iconBtn, { backgroundColor: colors.secondary, borderRadius: 10 }]}
            >
        ...
        {/* Today summary */}
        {todayTotal > 0 && (
          <View style={[styles.todaySummary, { backgroundColor: colors.primary + "12", borderRadius: 12 }]}>
            <Feather name="zap" size={14} color={colors.primary} />
            <Text style={[styles.todaySummaryText, { color: colors.primary }]}>
              Today: {formatMinutes(todayTotal)} studied
            </Text>
          </View>
        )}
          ...
          {dateStrip.map((day) => (
            ...
            >
              <Text
                style={[
                  styles.dateItemDay,
                  { color: day.isSelected ? "#fff" : colors.mutedForeground },
                ]}
              >
                {day.label}
              </Text>
              <Text
                style={[
                  styles.dateItemNum,
                  { color: day.isSelected ? "#fff" : colors.foreground },
                ]}
              >
                {day.dayNum}
              </Text>
              {day.hasStudy && !day.isSelected && (
                <View style={[styles.studyDot, { backgroundColor: colors.primary }]} />
              )}
            </TouchableOpacity>
          ))}
        </ScrollView>
        ...
        {showUndoBanner && lastSavedSession && (
          <View style={[styles.undoBanner, { backgroundColor: colors.foreground, borderRadius: 12 }]}>
            ...
            <View style={styles.undoActions}>
              <TouchableOpacity
                ...
                style={[styles.undoBtn, { backgroundColor: "#fff20" }]}
              >
        ...
        {subjects.length === 0 ? (
          <View style={[styles.emptyBox, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <TouchableOpacity
              ...
              style={[styles.emptyBtn, { backgroundColor: colors.primary, borderRadius: 10 }]}
            >
          ...
          subjects.map((subject) => {
            const displayMinutes = isToday
              ? getTodayMinutes(subject.id)
              : getMinutesForDate(subject.id, selectedDate);
            return (
              <SubjectCard
                key={subject.id}
                id={subject.id}
...
const styles = StyleSheet.create({
  ...
  todaySummaryText: { fontSize: 13, fontFamily: "Inter_600SemiBold" },
  ...
  dateItemDay: { fontSize: 10, fontFamily: "Inter_500Medium", textTransform: "uppercase", letterSpacing: 0.3 },
  ...
  emptyBtn: { paddingHorizontal: 20, paddingVertical: 10, marginTop: 8 },
